...
def test_run():
    ...
    df = pd.read_csv('/home/samira/Projects/Thesis/src/incremental_decision_tree-CART-Random_Forest_python-master/dataset/bank.csv',header=0,sep=';')
    # df = pd.read_csv('/home/samira/Projects/Thesis/src/incremental_decision_tree-CART-Random_Forest_python-master/dataset/default_of_credit_card_clients.csv,header=0,sep=',')
    ...
    #$$$$$ for default_of_credit_card_clients dataset
    #df=df.astype('int64')
  ...
  # y_test =np.reshape(y_test,(y_test.shape[0],1))
    ...
    index_num_v = numeric_columns(array)
    index_nom_v = nominal_columns(array)
   
    #$$$$$$$$$$$$$$$$$$$$$ for global_workspace_delete
    if len(index_num_v) > 1 and len(index_nom_v)<1:
    
        r1 , r2 = random.sample(index_num_v, 2)
        r3 , r4 = random.sample(index_num_v, 2)
        r5 ,  r6 =random.sample(index_num_v, 2)
        r7 ,r8 =random.sample(index_num_v, 2)
        r9 , r10 = random.sample(index_num_v, 2)
        r11 ,r12 = random.sample(index_num_v, 2)
        rand13=rand14=rand15=rand16=rand17=rand18 = -1
        list_column = [r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12]## columns we use for operate
        s = list(set (list_column))
        l = len(s)
   
        features = np.delete(features, s , 0)
        m = 6 
        for j in range(m):
            features = np.insert(features,0,str(j))
        features= features.tolist()
       
  
    if len(index_nom_v)>1 and len(index_num_v)<1:
     
        rand13 , rand14 = random.sample(index_nom_v, 2)
        rand15 , rand16 = random.sample(index_nom_v, 2)
        rand17 , rand18 = random.sample(index_nom_v, 2)
        r1=r2=r3=r4=r5=r6=r7=r8=r9=r10=r11=r12 = -1
        list2_column =[rand13,rand14,rand15,rand16,rand17,rand18]
        s_ = list(set (list2_column))
        l = len(s_)
        print("s",s_)
        features = np.delete(features, s_, 0)
        m = 3 
        for j in range(m):
            features =np.insert(features,0,str(j))
    ...
    if len(index_num_v)>1 and len(index_nom_v)>1:
        ...
        rand15 , rand16 = random.sample(index_nom_v, 2)
        rand17 , rand18 = random.sample(index_nom_v, 2)
        list_columns = [r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,rand13,rand14,rand15,rand16,rand17,rand18]
        s__ = list(set (list_columns))
        l = len(s__)
       
        features = np.delete(features, s__ , 0)
        m = 9 
        for j in range(m):
            features = np.insert(features,0,str(j))
        features= features.tolist()
    ...
    for training_set in examples:
        n += len(training_set)
        gw.dataset(training_set)
        test_set = df.tail(n_test).values
        train_set = np.append(training_set,test_set,axis=0)
        train_set = global_workspace_delete(train_set,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,rand13,rand14,rand15,rand16,rand17,rand18)
        test_set = train_set[-n_test:,:]
        x_test = test_set[:, :-1] 
        y_test = test_set[:, -1]
        training_set = train_set[:-n_test,:]
